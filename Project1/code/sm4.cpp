#include <chrono>
#include <random>
#include <vector>
#include <cstring>
#include <iomanip>
#include <iostream>
#include <cstdint>

using namespace std;

// 混淆盒
static const uint8_t ConfusionBox[256] = {
    
};

static const uint8_t MasterKey[16] = {
    0x01,0x23,0x45,0x67, 0x89,0xab,0xcd,0xef,
    0xfe,0xdc,0xba,0x98, 0x76,0x54,0x32,0x10
};

static const uint32_t FixedKeys[4] = {
    0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
};

static const uint32_t RoundConstants[32] = {
    0x00070e15,0x1c232a31,0x383f464d,0x545b6269,
    0x70777e85,0x8c939aa1,0xa8afb6bd,0xc4cbd2d9,
    0xe0e7eef5,0xfc030a11,0x181f262d,0x343b4249,
    0x50575e65,0x6c737a81,0x888f969d,0xa4abb2b9,
    0xc0c7ced5,0xdce3eaf1,0xf8ff060d,0x141b2229,
    0x30373e45,0x4c535a61,0x686f767d,0x848b9299,
    0xa0a7aeb5,0xbcc3cad1,0xd8dfe6ed,0xf4fb0209,
    0x10171e25,0x2c333a41,0x484f565d,0x646b7279
};

// 循环移位操作
static inline uint32_t circular_shift(uint32_t value, int bits) {
    return (value << bits) | (value >> (32 - bits));
}

// 数据混淆函数
static inline uint32_t nonlinear_transform(uint32_t word) {
    return (ConfusionBox[(word >> 24) & 0xFF] << 24) |
        (ConfusionBox[(word >> 16) & 0xFF] << 16) |
        (ConfusionBox[(word >> 8) & 0xFF] << 8) |
        ConfusionBox[word & 0xFF];
}

// 扩散函数
static inline uint32_t diffusion_operation(uint32_t word) {
    return word ^ circular_shift(word, 2) ^
        circular_shift(word, 10) ^
        circular_shift(word, 18) ^
        circular_shift(word, 24);
}

// 密钥生成
void generate_round_keys(const uint8_t key[16], uint32_t round_keys[32]) {
    uint32_t key_buffer[36];

    // 初始化密钥
    for (int idx = 0; idx < 4; idx++) {
        key_buffer[idx] = ((uint32_t)key[4 * idx] << 24) |
            ((uint32_t)key[4 * idx + 1] << 16) |
            ((uint32_t)key[4 * idx + 2] << 8) |
            key[4 * idx + 3];
        key_buffer[idx] ^= FixedKeys[idx];
    }

    // 生成轮密钥
    for (int idx = 0; idx < 32; idx++) {
        uint32_t mix = key_buffer[idx + 1] ^
            key_buffer[idx + 2] ^
            key_buffer[idx + 3] ^
            RoundConstants[idx];
        uint32_t transformed = nonlinear_transform(mix);
        round_keys[idx] = key_buffer[idx] ^ transformed ^
            circular_shift(transformed, 13) ^
            circular_shift(transformed, 23);
        key_buffer[idx + 4] = round_keys[idx];
    }
}

// 基础加密过程
void encrypt_block_basic(const uint8_t input[16], uint8_t output[16], const uint32_t round_keys[32]) {
    uint32_t state[36];

    // 加载数据
    for (int idx = 0; idx < 4; idx++) {
        state[idx] = ((uint32_t)input[4 * idx] << 24) |
            ((uint32_t)input[4 * idx + 1] << 16) |
            ((uint32_t)input[4 * idx + 2] << 8) |
            input[4 * idx + 3];
    }

    // 执行32轮加密
    for (int round = 0; round < 32; round++) {
        uint32_t combined = state[round + 1] ^
            state[round + 2] ^
            state[round + 3] ^
            round_keys[round];
        uint32_t substituted = nonlinear_transform(combined);
        uint32_t transformed = diffusion_operation(substituted);
        state[round + 4] = state[round] ^ transformed;
    }

    // 输出结果
    for (int idx = 0; idx < 4; idx++) {
        uint32_t value = state[35 - idx];
        output[4 * idx] = (value >> 24) & 0xFF;
        output[4 * idx + 1] = (value >> 16) & 0xFF;
        output[4 * idx + 2] = (value >> 8) & 0xFF;
        output[4 * idx + 3] = value & 0xFF;
    }
}

// CBC模式加密
void cbc_encrypt(const vector<uint8_t>& plain_data, vector<uint8_t>& cipher_data,
    const uint32_t round_keys[32], const uint8_t initialization_vector[16]) {
    size_t blocks = plain_data.size() / 16;
    cipher_data.resize(plain_data.size());

    uint8_t prev_cipher[16];
    copy(initialization_vector, initialization_vector + 16, prev_cipher);

    for (size_t block_idx = 0; block_idx < blocks; block_idx++) {
        uint8_t mixed_block[16];
        const uint8_t* cur_block = &plain_data[block_idx * 16];

        // 异或操作
        for (int byte_idx = 0; byte_idx < 16; byte_idx++) {
            mixed_block[byte_idx] = cur_block[byte_idx] ^ prev_cipher[byte_idx];
        }

        // 加密块
        encrypt_block_basic(mixed_block, &cipher_data[block_idx * 16], round_keys);

        // 更新前一块密文
        copy(&cipher_data[block_idx * 16], &cipher_data[block_idx * 16] + 16, prev_cipher);
    }
}

int main() {
    // 生成轮密钥
    uint32_t round_keys[32];
    generate_round_keys(MasterKey, round_keys);

    // 创建初始化向量
    uint8_t initialization_vector[16];
    mt19937 rng(random_device{}());
    uniform_int_distribution<uint16_t> dist(0, 255);
    for (int i = 0; i < 16; i++) {
        initialization_vector[i] = static_cast<uint8_t>(dist(rng));
    }

    // 准备明文
    string original_text = "WAANDA";
    size_t orig_length = original_text.length();
    size_t padded_length = (orig_length + 15) & ~15;
    vector<uint8_t> padded_data(padded_length);
    memcpy(padded_data.data(), original_text.data(), orig_length);

    // 填充数据
    uint8_t padding_value = static_cast<uint8_t>(padded_length - orig_length);
    for (size_t i = orig_length; i < padded_length; i++) {
        padded_data[i] = padding_value;
    }

    // 执行加密
    vector<uint8_t> encrypted_data;

    // 计时开始
    auto start_time = chrono::high_resolution_clock::now();

    cbc_encrypt(padded_data, encrypted_data, round_keys, initialization_vector);

    // 计时结束
    auto end_time = chrono::high_resolution_clock::now();
    double elapsed = chrono::duration<double, milli>(end_time - start_time).count();

    // 输出结果

    cout << "\nCipher: ";
    for (auto byte : encrypted_data) {
        cout << hex << setw(2) << setfill('0') << static_cast<int>(byte);
    }

    cout << "initialization vector: ";
    for (int i = 0; i < 16; i++) {
        cout << hex << setw(2) << setfill('0') << static_cast<int>(initialization_vector[i]);
    }
    cout << "\nBasic SM4 time: " << fixed << setprecision(3) << elapsed << " ms\n";

    return 0;
}