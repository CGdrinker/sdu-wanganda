...
// T-table优化的加密过程
void encrypt_block_ttable(const uint8_t input[16], uint8_t output[16], const uint32_t round_keys[32]) {
    uint32_t state[36];
    ...
    for (int round = 0; round < 32; round++) {
        uint32_t combined = state[round + 1] ^
            ...
            state[round + 3] ^
            round_keys[round];
        // 使用T-table进行变换，替代原来的nonlinear_transform和diffusion_operation
...
void cbc_encrypt(const vector<uint8_t>& plain_data, vector<uint8_t>& cipher_data,
    ...
    for (size_t block_idx = 0; block_idx < blocks; block_idx++) {
        ...
        // 使用T-table优化的加密函数
        encrypt_block_ttable(mixed_block, &cipher_data[block_idx * 16], round_keys);
...
int main() {
    ...
    // 输出结果
    cout << "=== SM4 T-table Optimization ===" << endl;
    cout << "IV: ";
    for (int i = 0; i < 16; i++) {
