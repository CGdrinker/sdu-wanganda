...
int main() {
    ...
    if (!check_gfni_support()) {
        cerr << "错误: 此CPU不支持GFNI指令集" << endl;
        cerr << "注意: GFNI需要Intel Ice Lake(第10代)及以后的CPU支持" << endl;
        return 1;
    }
    ...
    cout << dec << "\n数据大小: " << fixed << setprecision(3) << data_size_mb << " MB" << endl;
    cout << "加密时间: " << fixed << setprecision(3) << elapsed << " ms" << endl;
    cout << "加密速度: " << fixed << setprecision(3) << speed_mb_per_sec << " MB/s" << endl;
